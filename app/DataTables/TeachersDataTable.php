...
class TeachersDataTable extends DataTable {
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('designation', fn($query) => $query->designation?->title ?? '-')
            ->editColumn('phone', fn($query) => $query->phone ?? '-')
            ->editColumn('email', fn($query) => $query->email ?? '-')
            ->addColumn('photo', function ($query)
            {
                if ($query->images->isNotEmpty())
                {
                    return '<img src="' . asset('storage/images/' . $query->images[0]->type . '/' . $query->images[0]->filename) . '" alt="' . $this->title . ' Image" style="height: 90px; object-fit: cover;">';
                }
                return '-';
            })
    ...
    public function getColumns(): array
    {
        $columns = [
            ...
            Column::make('designation')->title(trans('backend.designation')),
            Column::make('email')->title(trans('backend.email')),
            Column::make('phone')->title(trans('backend.phone')),
            Column::make('photo')->title(trans('backend.profile_image')),
