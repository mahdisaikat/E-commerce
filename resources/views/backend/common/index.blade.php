                    ...
                    <div class="modal-content">
                        ...
                        <form id="{{ $action }}">
                                        ...
                                        <div class="{{ count($formFields) > 4 ? 'col-md-6' : 'col-md-12' }}">
                                            <div class="form-group mb-2">
                                                @if(!empty($field['label']))
                                                    <label for="{{ $action }}_{{ $field['name'] }}" class="form-label">{{ $field['label'] }}</label>
                                                @endif

                                                @if($field['type'] == 'textarea')
                                                    <textarea name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                              class="form-control {{ $field['name'] }} @error($field['name']) is-invalid @enderror"></textarea>

                                                @elseif($field['type'] == 'select')
                                                    <select name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                ...
                                                @elseif($field['type'] == 'checkbox')
                                                    <div class="form-check">
                                                        <input type="checkbox" name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                               value="1"
                                                               class="form-check-input {{ $field['name'] }} @error($field['name']) is-invalid @enderror">
                                                        <label class="form-check-label" for="{{ $action }}_{{ $field['name'] }}">
                                                ...
                                                @elseif($field['type'] == 'date')
                                                    <input type="date" name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                           class="form-control {{ $field['name'] }} @error($field['name']) is-invalid @enderror"
                                                           value="{{ date('Y-m-d') }}">

                                                @elseif($field['type'] == 'number')
                                                    <input type="number" name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                           class="form-control {{ $field['name'] }} @error($field['name']) is-invalid @enderror">

                                                @elseif($field['type'] == 'default')
                                                    <input type="hidden" name="{{ $field['name'] }}" id="{{ $action }}_{{ $field['name'] }}"
                                                           class="form-control {{ $field['name'] }} @error($field['name']) is-invalid @enderror"
                                                           value="{{ $field['value'] }}">
                            ...
                            <div class="modal-footer justify-content-between">
                                ...
                                <button type="submit" class="btn btn-success btn-sm">{{ $action == 'add' ? __('backend.save') :
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (response.type === 'success') {
                                ...
                                const modalInstance = coreui.Modal.getInstance(modalElement) || new coreui.Modal(modalElement);
                                modalInstance.hide();
                    ...
                    success: function (data) {
                        ...
                        formFields.forEach(field => {
                            ...
                            if (field.type === 'select' && field.options && field.options[value]) {
                                ...
                                if (value) {
                                    // Format date (default value is YYYY-MM-DD)
